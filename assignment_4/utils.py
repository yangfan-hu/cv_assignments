...
def warp_image(img, H, output_shape, offset):
    """
    Warp an image using a homography matrix, supporting both grayscale and color images.
    """
    # Compute the inverse of the homography matrix
    Hinv = np.linalg.inv(H)
    
    # Extract 2D transformation components
    m = Hinv.T[:2, :2]
    b = Hinv.T[:2, 2]
    
    if img.ndim == 3:
        # For color images (H, W, 3):
        # Extend the 2D transformation to 3D. 
        # The 3rd dimension (channels) remains unchanged (identity mapping).
        m_3d = np.eye(3)
        m_3d[:2, :2] = m
        
        # Extend the offset to 3D, adding zero shift for the channel axis
        b_3d = np.zeros(3)
        b_3d[:2] = b + offset
        
        # Ensure the output_shape includes the channel dimension
        out_shape_full = (output_shape[0], output_shape[1], img.shape[2])
        
        img_warped = affine_transform(
            img.astype(np.float32),
            m_3d, 
            b_3d,
            output_shape=out_shape_full,
            order=1, 
            cval=-1
        )
    else:
